...
document.getElementById("deleteButton").addEventListener("click", async () => {
    ...
    try {
        const response = await fetch(URL +"/delete_files/", {
            method: "DELETE",
        });
...
document.getElementById("processButton").addEventListener("click", async () => {
    ...
    try {
        ...
        const response = await fetch(URL +"/procesar/", {
            method: "POST",
        ...
        const intervalId = setInterval(async () => {
            try {
                const estadoResponse = await fetch(URL +"/estado_proceso/");
                ...
                console.log(lastMessage); // Debug: revisar el último mensaje
...
document.getElementById("showResultsButton").addEventListener("click", async () => {
    ...
    try {
        const response = await fetch(URL +"/mostrar_resultados/");
        const data = await response.json();

        console.log(data); // Debug: revisar la estructura de la respuesta

         // Verificar que data.requisitos_procesados existe y es un objeto
        if (response.ok && data.requisitos_procesados && Array.isArray(data.requisitos_procesados.requisitos)) {
            // Crear un contenedor para los requisitos procesados
            const requisitosContainer = document.createElement("div");
            requisitosContainer.className = "mb-3";


            // Crear una lista de requisitos
            const requisitosList = document.createElement("ul");
            requisitosList.className = "list-group mb-2";

            data.requisitos_procesados.requisitos.forEach((requisito, index) => {
                const listItem = document.createElement("li");
                listItem.className = "list-group-item";
                listItem.textContent = `R${index + 1} = ${requisito}`;
                requisitosList.appendChild(listItem);
            });

            requisitosContainer.appendChild(requisitosList);
            resultadosContainer.appendChild(requisitosContainer); // Agregar el contenedor de requisitos procesados al contenedor de resultados
        }
        ...
        if (response.ok && Array.isArray(data.resultados)) {
            data.resultados.forEach((resultado, index) => {
                ...
                const labels = resultado.requisitos_validados.map((_, i) => `R${i + 1}`);

                // Calcular la suma total de requisitos_validados
                const score = resultado.requisitos_validados.reduce((acc, curr) => acc + curr, 0);
